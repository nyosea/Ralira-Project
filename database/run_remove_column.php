...
$drop_index_sql = "DROP INDEX IF EXISTS idx_psychologist_date_selected ON psychologist_schedule_dates";
if ($conn->query($drop_index_sql)) {
    echo "Index 'idx_psychologist_date_selected' dropped successfully.\n";
} else {
    echo "Index 'idx_psychologist_date_selected' not found or already dropped.\n";
}

// Drop the column
$drop_column_sql = "ALTER TABLE psychologist_schedule_dates DROP COLUMN IF EXISTS is_date_selected";
if ($conn->query($drop_column_sql)) {
    echo "Column 'is_date_selected' dropped successfully.\n";
} else {
    echo "Column 'is_date_selected' not found or already dropped.\n";
}
