...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    $bio = trim($_POST['bio'] ?? '');
    $bersedia_konsul = trim($_POST['bersedia_konsul'] ?? 'Offline & Online');
    ...
    if (!$name || !$email || !$password || !$sipp) {
        ...
        if ($existing) {
            ...
            if ($db->executePrepare($sql_user, [$name, $email, $phone, $password_hash, 'psychologist'])) {
                ...
                $sql_profile = "INSERT INTO psychologist_profiles (user_id, spesialisasi, nomor_sipp, bio, foto_profil, pengalaman_tahun, bersedia_konsul) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)";
                $db->executePrepare($sql_profile, [$user_id, $spesialisasi, $sipp, $bio, $photo_path, $pengalaman, $bersedia_konsul]);
...
$sql_psychologists = "SELECT u.user_id, u.name, u.email, u.phone, pp.spesialisasi, pp.nomor_sipp, pp.pengalaman_tahun, pp.bersedia_konsul 
                     FROM users u
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                                ...
                                <div>
                                    ...
                                    <div style="margin-bottom: 15px;">
                                        ...
                                        <select name="spesialisasi" style="width: 100%; padding: 12px 15px; border: 2px solid #e1e8ed; border-radius: 8px; box-sizing: border-box; font-size: 14px; transition: all 0.3s ease; cursor: pointer;">
                                            <option value="Psikolog Anak">Psikolog Anak</option>
                                            <option value="Psikolog Dewasa">Psikolog Dewasa</option>
                                    ...
                                    <div style="margin-bottom: 15px;">
                                        <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #2c3e50;">
                                            <i class="fas fa-calendar-check" style="margin-right: 8px; color: var(--color-primary);"></i>Bersedia Konsultasi
                                        </label>
                                        <select name="bersedia_konsul" style="width: 100%; padding: 12px 15px; border: 2px solid #e1e8ed; border-radius: 8px; box-sizing: border-box; font-size: 14px; transition: all 0.3s ease; cursor: pointer;">
                    ...
                    <div class="psychologists-cards" id="psychologistsGrid">
                        ...
                        <div class="psychologist-card">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                                ...
                                <!-- Status Indicator -->
                                <div style="width: 12px; height: 12px; background: <?php 
                                    $status = $psi['bersedia_konsul'] ?? 'Offline & Online';
                                    if ($status === 'Tidak Tersedia') echo '#dc3545';
                                    elseif ($status === 'By Appointment') echo '#ffc107';
                                    else echo '#28a745';
                                ?>; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 8px rgba(<?php 
                                    $status = $psi['bersedia_konsul'] ?? 'Offline & Online';
                                    if ($status === 'Tidak Tersedia') echo '220, 53, 69';
                                    elseif ($status === 'By Appointment') echo '255, 193, 7';
                                    else echo '40, 167, 69';
                                ?>, 0.3);" title="<?php echo htmlspecialchars($psi['bersedia_konsul'] ?? 'Offline & Online'); ?>"></div>
                            </div>
